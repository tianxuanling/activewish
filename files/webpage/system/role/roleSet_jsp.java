...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
      ...
      out.write("\t\tvar prevNode = ''; //保存上一步所选父节点\r\n");
      out.write("\t\tfor ( var i = 0; i < node.length; i++) {\r\n");
      out.write("\t\t\tif ($('#functionid').tree('isLeaf', node[i].target)) {\r\n");
      out.write("\t\t\t\tcnodes += node[i].id + ',';\r\n");
      out.write("\t\t\t\tvar pnode = $('#functionid').tree('getParent', node[i].target); //获取当前节点的父节点\r\n");
      out.write("\t\t\t\tif (prevNode != pnode.id) //保证当前父节点与上一次父节点不同\r\n");
      out.write("\t\t\t\t{\r\n");
      out.write("\t\t\t\t\tpnodes += pnode.id + ',';\r\n");
      out.write("\t\t\t\t\tprevNode = pnode.id; //保存当前节点\r\n");
      out.write("\t\t\t\t}\r\n");
      out.write("\t\t\t}\r\n");
      out.write("\t\t}\r\n");
